...
with tabs[2]:
    ...
    # --- 2. INSTALLATION LOGS VIEW (BULLETPROOF GROUPED & FILTERED) ---
    with t_view_logs:
        df = get_data("WorkLogs")
        if not df.empty:
            df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
            
            # Extract standard columns ensuring they exist
            ...
            # Every DTR installation starts with a Box. This groups adjacent Box, Cable, Lugs seamlessly
            # even if DTR Code and SS No are totally blank.
            df['DateStr'] = df['Date'].dt.strftime('%Y-%m-%d')
            meta_cols = ['DateStr', 'Site', 'Worker', id_col]
            ...
            # A new group begins if metadata changes OR if a new Box is encountered
            ...
            if sel_box and box_col:
                filtered_df = filtered_df[filtered_df[box_col].astype(str).str.contains(sel_box, case=False, na=False)]
            if sel_ss and ss_col:
                filtered_df = filtered_df[filtered_df[ss_col].astype(str).str.contains(sel_ss, case=False, na=False)]
                
            if not filtered_df.empty:
                filtered_df['ItemDesc'] = filtered_df['Material'].astype(str) + " (" + filtered_df['Qty'].astype(str) + ")"
                
                # Group materials into a single row per installation group (_sub_grp)
                group_cols_to_agg = ['_sub_grp', 'DateStr', 'Worker', id_col]
                if ss_col: group_cols_to_agg.append(ss_col)
                if box_col: group_cols_to_agg.append(box_col)
                
                grouped = (
                    filtered_df.groupby(group_cols_to_agg, sort=False)
                    .agg({
