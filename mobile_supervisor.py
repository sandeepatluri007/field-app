...
with tabs[1]:
    ...
    with st.form("work_log", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 Submit Log", type="primary", use_container_width=True):
            batch_rows = []
            meta_data  = [str(w_date), w_main_id, w_dtr_box, w_ss_no, w_capacity, w_site, w_worker]
...
with tabs[2]:
    ...
    with t_survey_view:
        ...
        if not survey_data.empty:
            ...
            if surv_switch != "All" and 'LC/AB Switch' in filtered_surv.columns:
                filtered_surv = filtered_surv[
                    filtered_surv['LC/AB Switch'].astype(str).str.strip() == surv_switch
                ]
            if len(surv_date) == 2:
                mask = (
                    (filtered_surv['Date'].dt.date >= surv_date[0]) &
                    (filtered_surv['Date'].dt.date <= surv_date[1])
                )
                filtered_surv = filtered_surv[mask]
                
            if not filtered_surv.empty:
                filtered_surv = filtered_surv.copy()
                filtered_surv['Date'] = filtered_surv['Date'].dt.strftime('%Y-%m-%d')

                st.markdown(f"**{len(filtered_surv)} record(s)**")

                # ── Tiles ────────────────────────────────────────────────────
                selected_surv_indices = []
                for i, (_, row) in enumerate(filtered_surv.iterrows()):
                    dtr_code = str(row.get('DTR Code',     '')).strip()
                    dtr_ss   = str(row.get('DTR Name',     '')).strip()   # sheet col = DTR Name
                    ...
                    with st.container(border=True):
                        col_content, col_sel = st.columns([6, 1])

                        with col_content:
                            # Line 1: Date
                            st.markdown(f"**📅 {date_val}**")
                            # Line 2: DTR Code · DTR SS No
                            id_parts = []
                            if dtr_code: id_parts.append(f"🔌 **{dtr_code}**")
                            if dtr_ss:   id_parts.append(f"📡 {dtr_ss}")
                            if id_parts:
                                st.markdown("  ·  ".join(id_parts))
                            # Line 3: Switch · Lineman
                            meta_parts = []
                            if switch and switch != "None": meta_parts.append(f"🔘 {switch}")
                            if lineman:                      meta_parts.append(f"👤 {lineman}")
                            if meta_parts:
                                st.caption("  ·  ".join(meta_parts))
                            # Line 4: GPS · Synced badge
                            badge_parts = []
                            if lat and lon:
                                badge_parts.append(f"📍 {lat[:8]}, {lon[:8]}")
                            badge_parts.append("✅ Synced" if synced == "TRUE" else "🔄 Pending")
                            st.caption("  ·  ".join(badge_parts))

                        with col_sel:
                            if st.checkbox("", key=f"chk_surv_{i}", label_visibility="collapsed"):
                                selected_surv_indices.append(i)

                # ── Bulk actions ─────────────────────────────────────────────
                if selected_surv_indices:
                    selected_surv_df = filtered_surv.iloc[selected_surv_indices]
                    st.markdown(f"### ⚡ Actions for {len(selected_surv_indices)} Selected Record(s)")
                    ca1, ca2, ca3 = st.columns(3)
                    with ca1:
                        if st.button(f"🗑️ Delete {len(selected_surv_df)} Logs", key="del_surv"):
                            if bulk_delete_rows("SurveyLogs", selected_surv_df['ID'].tolist()):
                                st.rerun()
                    with ca2:
                        msg = "*Survey Details*\n"
                        for _, r in selected_surv_df.iterrows():
                            ...
                            loc_link = (
                                f"https://maps.google.com/?q={lat_r},{lon_r}"
                                if lat_r and lon_r else "No GPS recorded."
                            )
                            msg += (
                                f"\n➖ DTR SS No: {r['DTR Name']}\n"
                                f"DTR Code: {r['DTR Code']}\n"
                                f"Switch Type: {r.get('LC/AB Switch', 'None')}\n"
                                f"Lineman: {r.get('Lineman Name', 'N/A')}\n"
                                f"Location: {loc_link}\n"
                            )
                    ...
                    with ca3:
                        if FPDF:
                            ...
                            st.download_button(
                                ...
                                file_name="Selected_Survey_Logs.pdf",
                ...
                filtered_surv['label'] = (
                    filtered_surv['Date'].astype(str) + " | " +
                    filtered_surv['DTR Code'].astype(str) + " (" +
                    filtered_surv['DTR Name'].astype(str) + ")"
                )
                edit_sel_surv = st.selectbox(
                    "Select Record", [""] + filtered_surv['label'].tolist(), key="edit_sel_surv"
                )
                if edit_sel_surv:
                    ...
                    with st.form("edit_surv_form"):
                        ...
                        if st.form_submit_button("💾 Save Changes"):
                            if n_lc and n_ab:
                                ...
                                u_data = {
                                    "Date": n_date, "DTR Name": n_name, "DTR Code": n_code,
                                    "Latitude": n_lat, "Longitude": n_lon,
                                    "LC/AB Switch": switch_val, "Lineman Name": n_lineman,
                                    "Synced": "FALSE"
                                }
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 2. INSTALLATION LOGS — TILE DESIGN
    #    Tile structure (top → bottom):
    #      📅 Date  ·  👷 Worker
    #      🔌 DTR Code / SC No  ·  📡 DTR SS No  [·  📦 Box No]
    #      🔧 Materials Consumed
    # ─────────────────────────────────────────────────────────────────────────
    with t_view_logs:
        ...
        if not df.empty:
            ...
            if len(sel_date) == 2:
                mask = (
                    (filtered_df['Date'].dt.date >= sel_date[0]) &
                    (filtered_df['Date'].dt.date <= sel_date[1])
                )
            ...
            if sel_box and 'DTR_Box_No' in filtered_df.columns:
                filtered_df = filtered_df[
                    filtered_df['DTR_Box_No'].astype(str).str.contains(sel_box, case=False, na=False)
                ]
            if sel_ss and 'Transformer_SS_No' in filtered_df.columns:
                filtered_df = filtered_df[
                    filtered_df['Transformer_SS_No'].astype(str).str.contains(sel_ss, case=False, na=False)
                ]
                
            if not filtered_df.empty:
                filtered_df['DateStr']  = filtered_df['Date'].dt.strftime('%Y-%m-%d')
                filtered_df['ItemDesc'] = (
                    filtered_df['Material'].astype(str) + " (" +
                    filtered_df['Qty'].astype(str) + ")"
                )
                ...
                # ── Tiles ────────────────────────────────────────────────────
                st.markdown(f"**{len(grouped)} installation(s)**")
                ...
                for i, (_, row) in enumerate(grouped.iterrows()):
                    ...
                    dtr_ss    = str(row.get('DTR SS No',           '')).strip()
                    box_no    = str(row.get(box_col,               '')).strip() if box_col and box_col in row.index else ''
                    materials = str(row.get('Materials Consumed',  '')).strip()
                    row_ids   = row['ID']
                    ...
                    with st.container(border=True):
                        ...
                        with col_content:
                            ...
                            # Line 2 (NEW): DTR Code / SC No  ·  DTR SS No  [·  Box No]
                            id_parts = []
                            if dtr_code: id_parts.append(f"🔌 **{dtr_code}**")
                            if dtr_ss:   id_parts.append(f"📡 {dtr_ss}")
                            if box_no:   id_parts.append(f"📦 Box: {box_no}")
                            if id_parts:
                                st.markdown("  ·  ".join(id_parts))
                        ...
                        with col_sel:
                            if st.checkbox("", key=f"chk_wl_{i}", label_visibility="collapsed"):
                                selected_wl_ids.extend(row_ids)

                # ── Bulk delete ──────────────────────────────────────────────
                if selected_wl_ids:
                    n_selected = sum(
                        1 for i in range(len(grouped))
                        if st.session_state.get(f"chk_wl_{i}", False)
                    )
                    if st.button(f"🗑️ Delete {n_selected} Work Log(s)"):
                        if bulk_delete_rows("WorkLogs", selected_wl_ids):
                            st.rerun()

                st.markdown("---")

                # ── Single material edit ─────────────────────────────────────
                st.write("### ✏️ Edit Individual Material")
                st.caption("Select a specific material entry to edit its details.")
                filtered_df['label'] = (
                    filtered_df['DateStr'].astype(str) + " | " +
                    filtered_df[id_col].astype(str) + " (" +
                    filtered_df['Material'].astype(str) + ")"
                )
                edit_sel_wl = st.selectbox(
                    "Select Record to Edit", [""] + filtered_df['label'].tolist(), key="edit_sel_wl"
                )
                if edit_sel_wl:
                    ...
                    with st.form("edit_wl_form"):
                        ...
                        if st.form_submit_button("💾 Save Material Changes"):
                            u_data = {
                                "Date": n_date, id_col: n_dtr,
                                "Transformer_SS_No": n_ss, "DTR_Box_No": n_box,
                                "Worker": n_worker, "Material": n_mat, "Qty": n_qty,
                                "Synced": "FALSE"
                            }
    ...
    with t_inv_view:
        ...
        if not df_inv.empty:
            ...
            for i, (_, row) in enumerate(df_inv.iterrows()):
                ...
                with st.container(border=True):
                    ...
                    with col_content:
                        ...
                        # Line 2: Material · Qty  (same style as installation ID line)
                        mat_parts = []
                        if material: mat_parts.append(f"📦 **{material}**")
                        if qty:      mat_parts.append(f"🔢 {qty}")
                        if mat_parts:
                            st.markdown("  ·  ".join(mat_parts))

                        # Line 3: Synced badge
                        st.caption("✅ Synced" if synced == "TRUE" else "🔄 Pending sync")

                    with col_action:
                        if st.button("🗑️", key=f"del_inv_{i}", help="Delete this record"):
                            if bulk_delete_rows("Inventory", [row['ID']]):
                                st.success("Deleted!")
                                time.sleep(0.5)
                                st.rerun()
            ...
            df_inv['label'] = (
                df_inv['Date'].astype(str) + " | " +
                df_inv['Material'] + " (" +
                df_inv['Qty'].astype(str) + ")"
            )
            ...
            if edit_sel:
                ...
                with st.form("edit_inv_form"):
                    ...
                    n_mat  = st.selectbox(
                        ...
                        index=materials_list.index(sel_row['Material'])
                              if sel_row['Material'] in materials_list else 0
                    )
                    n_qty  = st.number_input("Qty", value=float(sel_row['Qty']))
                    if st.form_submit_button("💾 Save Changes"):
...
with tabs[4]:
    ...
    if not df_workers.empty:
        edited = st.data_editor(
            df_workers, use_container_width=True, num_rows="dynamic", key="w_edit",
